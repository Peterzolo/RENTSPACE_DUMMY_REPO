    ...
    const fetchAllUtilities = async () => {
        ...
        try {
            const response = await axios.get(
                'https://rentspacetech.com/api/utilities/all-utilities',
            ...
            console.log('results');
            console.log(results);
            setData(results.utilityHistories);
            //   localStorage.setItem('bvns', JSON.stringify(results.bvns));
            setIsLoading(false);
        } catch (error) {
            console.log('Error fetching utilities:', error.message);
            setIsLoading(false);
        }
    };
            ...
            <Card>
                ...
                <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
                    <table className="w-full min-w-[640px] table-auto">
                        <thead>
                            <tr>
                                {["Index", "User", "Description", "Status", "Amount", "Date"].map(
                                    (el) => (
                                        <th
                                            key={el}
                        ...
                        <tbody>
                            {data.map(
                                ({ user, description, amount, status, updatedAt, transactionReference }, key) => {
                                        ...
                                        <tr key={transactionReference + Math.random()}>
                                            ...
                                            <td className={className}>
                                                <Chip
                                                    ...
                                                    value={status.toLowerCase() === "pending" ? "Pending" : status.toLowerCase() === "failed" ? "Failed" : status.toLowerCase() === "submitted" ? "Submitted" : status.toLowerCase() === "cancelled" ? "Cancelled" : status.toLowerCase() === 'completed' || 'Success' || "successful"||"Completed" ||"success"|| "Successful" ? "Completed" : "Pending"}
