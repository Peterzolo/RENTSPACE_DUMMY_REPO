...
export function DVAManagement() {
    ...
    const fetchAllDVAs = async () => {
        ...
        try {
          const response = await axios.get(
            'https://rentspacetech.com/api/dva/get-dvas',
          ...
          console.log('results');
          console.log(results);
          setData(results.dvas);
        //   localStorage.setItem('dvas', JSON.stringify(results.dvas));
          setIsLoading(false);
        } catch (error) {
          console.log('Error fetching dva:', error.message);
          setIsLoading(false);
        }
    };
    ...
    const downloadCSV = () => {
        const csvContent = [
          "Index,User,Account Name,Account Number,Date",
          ...data.map(({  user, dvaName, dvaNumber, dvaDate,  }, index) => (
            `${index + 1},"${user?.lastName + " " + user?.firstName}","${dvaName}", "${dvaNumber}","${moment(dvaDate).format('DD/MM/YYYY')}"`
          ))
        ].join("\n");
    
        const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8' });
        saveAs(blob, 'virtual accounts.csv');
      };
  ...
  useEffect(() => {
    ...
    return (<div className="mt-12 mb-8 flex flex-col gap-12">
        {userData.role !== 'super-admin' && userData.role !== 'admin' && userData.role !== 'account' && userData.role !== 'marketing' ? <div className="flex items-center justify-normal mx-auto h-[50vh]">Oops!!! You do not have permission to view this Page.</div> :
            <Card>
            ...
            <button type="button" className="text-white bg-[#145182] hover:bg-[#145182] focus:outline-none  font-medium rounded-full text-sm px-16 py-3.5 text-center me-2 mb-2 dark:bg-blue-600 dark:hover:bg-[#145182] dark:focus:ring-[#145182]" onClick={downloadCSV}>Export Data</button>
                ...
                <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
                    <table className="w-full min-w-[640px] table-auto">
                        <thead>
                            <tr>
                                {["Index", "User", "Account Name", "Account Number", "Created On"].map(
                                    (el) => (
                                        <th
                                            key={el}
                        ...
                        <tbody>
                            {data.map(
                                ({ user, dvaName, dvaNumber, dvaDate, }, key) => {
                                        ...
                                        <tr key={dvaNumber + Math.random()}>
                                            ...
                                            <td className={className}>
                                                <Typography className="text-xs font-semibold text-blue-gray-600">
                                                    {moment(dvaDate).format('MMMM Do YYYY, h:mm:ss a')}
